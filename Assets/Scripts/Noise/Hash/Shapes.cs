    ...
    public static class Shapes
    {
        ...
        public struct Plane : IShape
        {
            public Point4 GetPoint4(int i, float resolution, float invResolution)
            {
                ...
                return new Point4
                    { positions = float4x3(uv.c0 - 0.5f, 0f, uv.c1 - 0.5f), normals = float4x3(0f, 1f, 0f) };
            }
        }
        ...
        public struct Sphere : IShape
        {
            public Point4 GetPoint4(int i, float resolution, float invResolution)
            {
                ...
                float4 scale = 0.5f * rsqrt(p.positions.c0 * p.positions.c0 + p.positions.c1 * p.positions.c1 +
        ...
        public struct Job<S> : IJobFor where S : struct, IShape
        {
            ...
            float4x3 TransfromVectors(float3x4 trs, float4x3 p, float w = 1f) => float4x3(
            ...
            public void Execute(int i)
            {
                ...
                positions[i] = transpose(TransfromVectors(positionTRS, p.positions));

                float3x4 n = transpose(TransfromVectors(normalTRS, p.normals, 0f));
            ...
            public static JobHandle ScheduleParallel(NativeArray<float3x4> positions, NativeArray<float3x4> normals, int resolution, float4x4 trs, JobHandle dependency)
            {
                ...
                return new Job<S>
                    {
                        positions = positions, normals = normals, resolution = resolution, invResolution = 1f / resolution,
